...
declare v_key text;
begin
  -- FAIL CLOSED: if Vault is present the key MUST come from Vault. A missing/empty ai_evidence_key is a
  -- misconfiguration, not a licence to encrypt production evidence with a public fallback string.
  if wl_ai_vault_available() then
    ...
    if v_key is null or length(btrim(v_key)) = 0 then
      raise exception 'ai_evidence_key is not set in Vault; refusing to encrypt/decrypt evidence with a fallback key'
        using errcode = '55000';
    end if;
    return v_key;
  end if;
  -- Vault ENTIRELY ABSENT (the disposable CI database only): a clearly-labelled non-secret fallback so
  -- the encrypt/decrypt path is exercised in tests. Never reached in production, where Vault is present.
  return 'wl-evidence-fallback-key-ci-only-not-secret';
end $$;
