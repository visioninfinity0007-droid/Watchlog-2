...
class DahuaDriver(NvrDriver):
    ...
    def configure_push(self, url: str) -> dict:
        """Fail closed for generic Dahua AlarmServer configuration.

        Dahua's documented AlarmServer is a vendor alarm-centre protocol, not a
        generic HTTP/HTTPS webhook. Its protocol values are Dahua/Bosch/cloud
        families; writing an HTTPS WatchLog URL into AlarmServer.Address/Port can
        overwrite a customer's existing alarm-centre configuration without giving
        WatchLog a valid callback.

        Therefore the generic Dahua driver NEVER mutates AlarmServer for PC-off
        reporting. Normal Dahua production coverage remains:
          * native live eventManager monitoring while the site connector is running;
          * durable local spooling through Internet outages;
          * recorder archive enumeration/download + resumable gap backfill when
            recorder/PC connectivity returns.

        A future firmware-specific HTTP push adapter may override this only after
        real hardware proves its exact endpoint and payload. Until then direct
        NVR->cloud liveness is honestly unsupported on generic Dahua firmware.
        """
        try:
            kv = _parse_kv(self._get(
            ...
            proto = ""
        detail = (
            "generic Dahua AlarmServer is a proprietary alarm-centre protocol, "
