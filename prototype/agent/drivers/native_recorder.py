...
# These are recorder-side smart classifications, not generic pixel motion.
_DAHUA_NATIVE_AI = {
    "SmartMotionHuman",
    "SmartMotionVehicle",
    "CrossLineDetection",
...
class NativeDahuaDriver(DahuaDriver):
    ...
    def _parse_line(self, line: str) -> Event | None:
        ...
        for part in head.split(";"):
            ...
            if key.strip() == "Code":
                code = value.strip()
                break
        if code in _DAHUA_NATIVE_AI:
            return _native(ev, "dahua", code)
        payload = dict(ev.payload or {})
    ...
    def capabilities(self) -> dict:
        ...
        # Playback is a candidate until the actual recorder proves it. The
        # portal must use the explicit playback_probe flag from field evidence,
        # not this class existing, to claim clip support as validated.
    ...
    def _device_clock_offset(self) -> float:
        """Approximate the recorder-local clock offset from UTC.

        Dahua playback/export parameters are interpreted in recorder local
        time. The live event stream does not include a trustworthy device
        timestamp, so derive the offset read-only from the recorder itself.
        If that endpoint is unavailable, fall back to the site PC's local
        timezone, which is normally configured to match the recorder.
        """
    ...
    def get_clip(self, channel: str, start: datetime, end: datetime) -> bytes | None:
        ...
        This is intentionally a pilot path: Dahua firmware families differ in
        how precisely ``loadfile.cgi`` trims a requested interval. A response
        larger than 32 MiB is aborted rather than allowing one incident to
        consume the site's uplink or database budget.
        """
        try:
            channel_no = int(str(channel))
        except (TypeError, ValueError):
            raise DriverError("invalid Dahua channel for incident footage")
        if end <= start:
            raise DriverError("invalid incident footage time window")

        url = self.base_url + "/cgi-bin/loadfile.cgi"
        params = {
            "action": "startLoad",
            "channel": channel_no,
            "startTime": self._recorder_time(start),
            "endTime": self._recorder_time(end),
            "subtype": 0,
        }
        try:
            response = self.s.get(
                ...
                params=params,
                stream=True,
                timeout=(CLIP_CONNECT_TIMEOUT, CLIP_READ_TIMEOUT),
            )
        ...
        try:
            ...
            if response.status_code >= 400:
                raise DriverError(f"incident footage export: HTTP {response.status_code}")
            chunks: list[bytes] = []
            total = 0
            for chunk in response.iter_content(chunk_size=256 * 1024):
                ...
                chunks.append(chunk)
            data = b"".join(chunks)
            # Dahua native recordings usually contain DHAV frames; some
            # firmware wraps them. Only reject obvious text/error responses.
            if not data:
                return None
            head = data[:256].lower()
            if b"error" in head or b"<!doctype html" in head or b"<html" in head:
                return None
            return data
        finally:
            response.close()
