...
def main():
    ...
    for rpc in (
        ...
        "wl_site_health_details",
        "wl_portal_snapshot",
    ):
        require(PAGE, f'rpc("{rpc}"', f"Control Room must reuse {rpc}")

    # Saved layout UI must use the dedicated tenant-scoped RPC surface.
    for rpc in (
        "wl_control_room_layouts",
        "wl_save_control_room_layout",
        "wl_delete_control_room_layout",
        "wl_camera_config_snapshot",
        "wl_request_config_snapshot",
    ):
        require(PAGE, f'rpc("{rpc}"', f"Control Room layout UI must use {rpc}")
    require(PAGE, "2 × 2", "2x2 layout option missing")
    require(PAGE, "3 × 3", "3x3 layout option missing")
    require(PAGE, "4 × 4", "4x4 layout option missing")
    require(PAGE, "Fullscreen operations", "fullscreen operations mode missing")
    require(PAGE, "<optgroup", "camera choices must be grouped by site")
    require(PAGE, "Request fresh still", "still refresh must be explicit operator action")
    if "setInterval(requestFreshStill" in PAGE or "setInterval(() => requestFreshStill" in PAGE:
        raise AssertionError("camera still requests must never run automatically on an interval")
