...
const PROVIDER_TYPES = ["ollama", "openai_compat", "openai", "anthropic", "gemini"];
    ...
    <section className={styles.card}>
      ...
      <form className={styles.form} onSubmit={saveProvider}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div><label>Type</label><select value={form.type} onChange={(e) => set("type", e.target.value)}>{PROVIDER_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}</select></div>
        </div>
