...
export default function Dashboard(){
  ...
  const silentItems=silentCameras.slice(0,2).map((c)=>c?.camera&&c?.site?`${c.camera} silent at ${c.site}`:c?.site?`Camera silent at ${c.site}`:null).filter(Boolean);
  const attentionItems=[...Object.entries(offBySite).map(([site,n])=>`${n} agent${n>1?"s":""} offline at ${site}`),...(silentItems.length?silentItems:(silentCameras.length?[`${silentCameras.length} camera${silentCameras.length>1?"s":""} silent`]:[])),...(faults24.length?[`${faults24.length} recorder or camera fault${faults24.length>1?"s":""} in the last 24h`]:[])];
  ...
  return <div className="shell"><Nav active="Overview" email={email} right={<><span className="muted hide-sm" style={{fontSize:"var(--font-size-xs)"}}>{stamp?`updated ${stamp}`:""}</span><select value={days} onChange={(e)=>setDays(Number(e.target.value))} style={{width:"auto",margin:0}} aria-label="Overview date range"><option value={1}>24 hours</option><option value={7}>7 days</option><option value={30}>30 days</option></select></>}/>
    <main className="main">
      ...
      <div className={"banner"+(attentionCount?"":" clear")}>{attentionCount?<><b>{attentionCount} thing{attentionCount>1?"s":""} need attention</b><ul>{attentionItems.slice(0,4).map((t,i)=><li key={i}>{t}</li>)}</ul></>:<><b>Everything is reporting normally.</b><div className="muted" style={{fontSize:"var(--font-size-sm)",marginTop:4}}>No offline agents, silent cameras or recent equipment faults were reported.</div></>}</div>

      <section className={ui.metricGrid} aria-label="Overview summary"><div className={ui.metric}><div className={ui.metricValue}>{data?.totals?.events??0}</div><div className={ui.metricLabel}>Events · {data?.window_days} days</div></div><div className={ui.metric}><div className={ui.metricValue}>{data?.totals?.cameras??0}</div><div className={ui.metricLabel}>Cameras</div></div><div className={ui.metric}><div className={ui.metricValue} style={{color:agents.length&&online===0?"var(--color-status-bad-dark)":online<agents.length?"var(--color-status-warn-dark)":agents.length?"var(--color-status-ok-dark)":undefined}}>{online} / {agents.length}</div><div className={ui.metricLabel}>Agents online</div></div><div className={ui.metric}><div className={ui.metricValue}>{data?.totals?.sites??0}</div><div className={ui.metricLabel}>{(data?.totals?.sites??0)===1?"Site":"Sites"}</div></div></section>

      <section className="overview-grid">
        <div className="panel overview-panel">
          <div className="target-panel-title"><div><h2>Sites &amp; agents</h2><p>Grouped by the real operating location.</p></div><a className="target-link" href="/settings/">Manage setup</a></div>
          <div className={ui.tableWrap}>{agents.length===0?<div className="empty">No Site Agent has connected yet. <a href="/settings/">Open Sites &amp; Setup</a> to connect one.</div>:<table><thead><tr><th>Site / agent</th><th>Status</th><th>Last seen</th><th>Events</th><th className="hide-sm">Recorder</th></tr></thead><tbody>{Object.entries(bySite).map(([site,list])=>{const anyOnline=list.some((a)=>liveness(a.last_seen_at)[0]==="s-ok"),siteEvents=list.reduce((s,a)=>s+(a.event_count||0),0);return <Fragment key={site}><tr className="overview-site-row"><td><b>{site}</b></td><td><span className={"pill "+(anyOnline?"s-ok":"s-bad")}>{anyOnline?"online":"offline"}</span></td><td className="muted">{list.length} agent{list.length===1?"":"s"}</td><td className="mono">{siteEvents}</td><td className="muted hide-sm"></td></tr>{list.map((a)=>{const[cls,label]=liveness(a.last_seen_at),device=[a.device_vendor,a.device_model].filter(Boolean).join(" ");return <tr className="overview-agent" key={a.agent_id}><td>{a.hostname||"unknown PC"}</td><td><span className={"pill "+cls}>{label}</span></td><td className="mono" title={exact(a.last_seen_at)}>{ago(a.last_seen_at)}</td><td className="mono">{a.event_count}</td><td className="muted hide-sm">{device||"not set"}</td></tr>;})}</Fragment>;})}</tbody></table>}</div>
        </div>

        <div className="overview-side">
          <div className="panel overview-panel">
            <div className="target-panel-title"><div><h2>Recent incidents</h2><p>Validated moments with stills.</p></div><a className="target-link" href="/incidents/">View all</a></div>
            {withShots.length===0?<div className="empty">No incident stills in this window yet.</div>:<div className="overview-shots">{withShots.map((e)=>{const shot=shots.current.get(e.event_id);return <a className="shot" key={e.event_id} href="/incidents/" style={{color:"inherit",textDecoration:"none"}}>{shot===undefined||shot===null?<div style={{display:"grid",placeItems:"center",background:"var(--color-canvas)"}} className="muted">loading still...</div>:shot===false?<div style={{display:"grid",placeItems:"center",background:"var(--color-canvas)"}} className="muted">still unavailable</div>:<img src={shot} alt={`still from ${e.camera||"camera"}`}/>}<div className="meta"><b>{e.camera||"unassigned"}</b><div className="muted">{humanType(e.event_type)}</div><div className="muted" title={exact(e.device_ts)}>{ago(e.device_ts)}</div></div></a>;})}</div>}
          </div>

          <div className="panel overview-panel">
            <div className="target-panel-title"><div><h2>Event types</h2><p>Distribution for the selected window.</p></div><span className="muted" style={{fontSize:12}}>{days===1?"24 hours":`${days} days`}</span></div>
