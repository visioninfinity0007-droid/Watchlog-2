...
const MAIN_TABS=[["Reports","/reports/"],["Setup","/setup/"]];
const MORE_TABS=[
...
const MORE_ACTIVE=new Set(["Control Room","Incidents","Site Health","Analytics","Archive","Settings"]);
const ACTIVE_ROUTE={
  "WatchLog AI":"/ai/",
  Reports:"/reports/",
  Setup:"/setup/",
...
export function Nav({active,email,right,currentSiteId=""}){
  ...
  useEffect(()=>{let live=true;(async()=>{
    const sb=supabase();
    const[p,s,c]=await Promise.all([sb.rpc("wl_platform_me"),sb.rpc("wl_sites"),sb.rpc("wl_ai_conversations",{p_limit:12})]);
    if(!live)return;
  ...
  const currentRoute=ACTIVE_ROUTE[active]||"/ai/";

  function siteHref(id){return withSite(currentRoute,id);}
  ...
  async function askWatchLog(e){
    e?.preventDefault();
    const prompt=askDraft.trim();
    if(!prompt||!siteId||askBusy)return;
    setAskBusy(true);setAskError("");setAskAnswer("");
    const{data,error}=await supabase().functions.invoke("watchlog-ai",{body:{prompt,site_id:siteId,conversation_id:null}});
    setAskBusy(false);
    if(error||data?.error){setAskError(data?.message||say(error)||"WatchLog could not answer that request.");return;}
    setAskAnswer(data?.answer||"WatchLog completed the request.");
  }
  ...
  return <>
    <aside className={`productRail ${mobileOpen?"open":""}`}>
      <div className="productRailMobileHead">
        ...
        <button className="productRailMenu" onClick={()=>setMobileOpen((v)=>!v)} aria-label="Open WatchLog navigation" aria-expanded={mobileOpen}>☰</button>
      </div>
      ...
      <div className="productRailSites">{sites.length?sites.map((s)=><a key={s.id} href={siteHref(s.id)} onClick={()=>chooseSite(s.id,s.name)} className={`productRailSite ${s.id===siteId?"active":""}`} aria-current={s.id===siteId?"true":undefined}><span>{s.name}</span><small>{s.online?"Monitoring":s.setup_state==="ready"?"Needs attention":"Setup required"}</small></a>):<a className="productRailSite" href="/setup/"><span>Add your first site</span><small>Start setup</small></a>}</div>

      <div className="productRailSectionLabel">Recent</div>
      <div className="productRailRecent">{conversations.slice(0,5).map((c)=><a key={c.id} href={`/ai/?site=${encodeURIComponent(c.site_id||siteId)}&conversation=${encodeURIComponent(c.id)}`} onClick={()=>c.site_id&&chooseSite(c.site_id,c.site_name||"")} className="productRailConversation"><span>{c.title||"WatchLog conversation"}</span><small>{c.site_name||"Site conversation"}</small></a>)}{!conversations.length&&<div className="productRailEmpty">Your recent conversations will appear here.</div>}</div>
      ...
      <nav className="productRailNav" aria-label="WatchLog navigation">
        ...
        {moreOpen&&<div className="productRailSubnav">{MORE_TABS.map(([label,href])=><a key={href} href={withSite(href,siteId)} className={"productRailSubLink"+(active===label||((active==="Control Room")&&label==="Camera View")||((active==="Archive")&&label==="Evidence")?" active":"")}>{label}</a>)}</div>}
      </nav>

      <div className="productRailBottom">
        <button className="productRailAsk" onClick={()=>{setAskOpen(true);setMobileOpen(false);}}><span>✦</span> Ask WatchLog</button>
    ...
    {askOpen&&<>
      <button className="watchlogAskScrim" onClick={()=>setAskOpen(false)} aria-label="Close WatchLog assistant"/>
      <aside className="watchlogAskDrawer" role="dialog" aria-modal="true" aria-label="Ask WatchLog">
        <div className="watchlogAskHead"><div><span>Ask WatchLog</span><b>{siteId?currentName:"Choose a site first"}</b></div><button onClick={()=>setAskOpen(false)} aria-label="Close">×</button></div>
        <div className="watchlogAskBody">{askAnswer?<div className="watchlogAskAnswer"><Mark size={24}/><p>{askAnswer}</p></div>:<div className="watchlogAskIntro"><Mark size={34}/><h3>Ask about what you're viewing.</h3><p>WatchLog uses the selected site's current monitoring, camera and incident information.</p></div>}{askError&&<div className="err">{askError}</div>}</div>
        <form className="watchlogAskComposer" onSubmit={askWatchLog}><textarea rows={2} value={askDraft} onChange={(e)=>setAskDraft(e.target.value)} placeholder={siteId?`Ask about ${currentName}...`:"Select a site first"} disabled={!siteId||askBusy}/><button aria-label="Send" disabled={!siteId||!askDraft.trim()||askBusy}>{askBusy?"…":"↑"}</button></form>
      </aside>
    </>}
  </>;
}
