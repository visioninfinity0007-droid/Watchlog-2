...
export default function CustomerReports(){
  ...
  return <div className="shell"><Nav active="Reports" email={r.email} currentSiteId={r.siteId}/><main className="main"><header className="target-page-head"><div><div className="target-eyebrow">Reports</div><h1>Management report</h1><p>A clear daily view of security, office activity and anything management should act on.</p></div><div className="target-actions"><a className={ui.secondaryLink} href={withSite("/reports/delivery/",r.siteId)}>Delivery</a></div></header>{r.error&&<div className="err">{r.error}</div>}<div className={ui.tabs}>{VIEWS.map(([k,l])=><button key={k} className={`${ui.tab} ${r.view===k?ui.tabActive:""}`} onClick={()=>r.setView(k)}>{l}</button>)}</div>{r.busy?<div className={ui.emptyCard}>Preparing your management report…</div>:r.view==="yesterday"?(r.snapshot?<ManagementReport snapshot={r.snapshot}/>:<div className={ui.emptyCard}>Yesterday's report is not available yet.</div>):<section className="daily-report-card"><div className="daily-report-head"><Mark size={26}/><b>{label} report</b></div><div className="daily-report-body">{r.answer?<RichText text={r.answer}/>:<p style={{margin:0}}>No report is available yet.</p>}</div></section>}<div className={ui.sectionHead}><div><h2>Want to go deeper?</h2><p>Ask WatchLog a question about this report.</p></div><a className={ui.primaryLink} href={withSite(`/ai/?prompt=${encodeURIComponent(prompt)}`,r.siteId)}>Ask WatchLog</a></div></main></div>;
}
