...
def validate_target(project_ref: str, db_host: str, db_user: str) -> None:
    ...
    if host.startswith("db.") and host.endswith(".supabase.co"):
        ...
        if host != expected_host:
            raise RuntimeError(f"database host does not match WatchLog project: {db_host!r}")
    user = db_user.strip()
    if user.startswith("postgres.") and user != f"postgres.{EXPECTED_PROJECT_REF}":
        raise RuntimeError(f"database user does not match WatchLog project: {db_user!r}")


def classify_boundary(markers: dict[str, Any]) -> str:
    """Return a conservative schema-boundary classification from read markers."""
    has_0023 = bool(markers.get("entitlement_0023")) and bool(markers.get("reporting_0023"))
    through_0036_keys = (
    ...
    ops_0037_keys = (
        "commercial_invoices_0037","support_sessions_0037","platform_commercial_0037",
    )
    lifecycle_0038_keys = (
        "account_status_0038","my_account_0038","account_lifecycle_0038",
    )
    control_room_0039_keys = (
        "control_room_layouts_0039","control_room_layout_rpc_0039",
    )
    footage_0040_keys = (
        "incident_clip_requests_0040","incident_clip_chunks_0040","incident_clip_agent_rpc_0040",
    )
    footage_0041_keys = ("incident_clip_fail_hardened_0041",)
    ...
    footage = [bool(markers.get(key)) for key in footage_0040_keys]
    hardening = [bool(markers.get(key)) for key in footage_0041_keys]
    any_after_0023 = any(old + ops + lifecycle + control_room + footage + hardening)

    if has_0023 and all(old):
        if all(ops) and all(lifecycle) and all(control_room):
            if all(footage) and all(hardening):
                return "0041_candidate_requires_footage_smoke"
            if not any(footage) and not any(hardening):
                return "0039_candidate_requires_authz_smoke"
            return "partial_after_0039_stop_and_reconcile"
        if all(ops) and all(lifecycle):
            if not any(control_room + footage + hardening):
                return "0038_exact_candidate"
            return "partial_after_0038_stop_and_reconcile"
        if all(ops) and not any(lifecycle + control_room + footage + hardening):
            return "0037_exact_candidate"
        if not any(ops + lifecycle + control_room + footage + hardening):
        ...
        try:
            ...
            markers = conn.execute("""
                select
                  ...
                  exists (select 1 from information_schema.columns where table_schema='public' and table_name='sites' and column_name='site_type') as site_type_0024,
                  exists (select 1 from information_schema.columns where table_schema='public' and table_name='cameras' and column_name='purpose') as camera_purpose_0024,
                  ...
                  exists (select 1 from information_schema.columns where table_schema='public' and table_name='monitoring_rules' and column_name='analytic_key') as analytic_key_0030,
                  exists (select 1 from information_schema.columns where table_schema='public' and table_name='report_recipients' and column_name='whatsapp_destination')
                    and exists (select 1 from information_schema.columns where table_schema='public' and table_name='report_recipients' and column_name='email_destination') as recipient_destinations_0031,
                  ...
                  (select count(*) = 4 from pg_policies where schemaname='public'
                     and policyname in ('owner_read_billing_customers','owner_read_subscriptions','owner_read_payment_transactions','owner_read_billing_checkouts')
                     and cmd='SELECT' and coalesce(qual,'') like '%wl_my_tenant%' and coalesce(qual,'') like '%wl_my_role%') as billing_owner_policies_0036,
                  ...
                  exists (select 1 from information_schema.columns where table_schema='public' and table_name='tenants' and column_name='account_status') as account_status_0038,
                  ...
                  to_regclass('public.incident_clip_requests') is not null as incident_clip_requests_0040,
                  to_regclass('public.incident_clip_chunks') is not null as incident_clip_chunks_0040,
                  to_regprocedure('public.wl_agent_claim_clip_requests(uuid,text,integer)') is not null as incident_clip_agent_rpc_0040,
                  coalesce(obj_description(to_regprocedure('public.wl_agent_fail_clip(uuid,text,uuid,text,boolean)')::oid,'pg_proc'),'') like '%0041%' as incident_clip_fail_hardened_0041
            """).fetchone()

            counts: dict[str, int | None] = {
                "auth_users": conn.execute("select count(*) as n from auth.users").fetchone()["n"],
                "tenants":None,"sites":None,"cameras":None,
            }
            for table in ("tenants","sites","cameras"):
                if markers[f"{table}_core"]:
                    counts[table] = conn.execute(f"select count(*) as n from public.{table}").fetchone()["n"]

            ledger_present = conn.execute("select to_regclass('public.schema_migrations') is not null as present").fetchone()["present"]
            ledger_columns: list[str] = []
            ledger_shape_ok = False
            ledger: list[dict[str, Any]] = []
            if ledger_present:
                ledger_columns = [row["column_name"] for row in conn.execute("""
                    select column_name from information_schema.columns
                    where table_schema='public' and table_name='schema_migrations'
                    order by ordinal_position
                """).fetchall()]
                ledger_shape_ok = {"filename","sha256","applied_at"}.issubset(set(ledger_columns))
                if ledger_shape_ok:
                    ledger = conn.execute("""
            ...
            if admin_email:
                auth_row = conn.execute("""
                    select id::text as user_id,email_confirmed_at is not null as email_confirmed
                    from auth.users where lower(email)=lower(%s) limit 1
                """,(admin_email,)).fetchone()
                admin = {"auth_user_exists":auth_row is not None,"email_confirmed":bool(auth_row and auth_row["email_confirmed"]),"platform_role":None}
                if auth_row and markers["platform_admins_0029"]:
                    role_row = conn.execute("select role from public.platform_admins where user_id=%s::uuid",(auth_row["user_id"],)).fetchone()
                    admin["platform_role"] = role_row["role"] if role_row else None

            boundary = classify_boundary(dict(markers))
            needs_reconcile = "stop_and_reconcile" in boundary or (bool(ledger_present) and not ledger_shape_ok)
            return {
                "tool":"watchlog_production_preflight","project_ref":project_ref,
                "identity":dict(identity),"read_only_proven":True,"markers":dict(markers),
                "boundary":boundary,"row_counts":counts,
                "migration_ledger":{"present":bool(ledger_present),"shape_ok":ledger_shape_ok if ledger_present else None,"columns":ledger_columns,"rows":ledger},
                "admin":admin,
                "next_action":"No DDL. Reconcile partial/unknown schema or migration-ledger shape first."
                    if needs_reconcile else "Use the guarded production parity runbook; this report alone never authorizes DDL.",
            }
        finally:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Read-only WatchLog production schema/data preflight")
    parser.add_argument("--admin-email",help="Optionally verify that this Auth user exists and inspect its platform role")
    parser.add_argument("--output",help="Optional JSON output file. Secrets are never included.")
