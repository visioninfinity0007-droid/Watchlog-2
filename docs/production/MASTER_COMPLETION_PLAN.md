...
| **P2** | Agent AI: build frozen exe **with** onnxruntime+numpy+Pillow+`yolov8n.onnx`; self-test proves inference; person/car/motorcycle only; fail-open | **DONE (code+model+source-verified)**; frozen-exe build = AI-6 verifying | model loads, inference runs, junk discarded, person retained — all ✅ via `--selftest`/`test_vision_onnx`; frozen-exe proof building |
